...
class MMAController(Controller):
    ...
    def choose_model(self, x, u, x_dot):
        ...
        error_list = np.sum(np.abs(error_list), axis=1)
    ...
    def calculate_control(self, x, q_d, q_d_dot, q_dd_dot):
        ...
        v = q_dd_dot + self.kds[self.i].dot(x[2:].reshape(-1, 1)-q_d_dot.reshape(-1, 1)) + self.kps[self.i].dot(x[:2].reshape(-1, 1)-q_d.reshape(-1, 1))
